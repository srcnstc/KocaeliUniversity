...
{ 
  ...
  for(i = 8; i > 0; i--)
  {
    ...
    DS_SCL0;
    delay(10);
    rdata >>= 1;                                 //okunan veri 1 bit saga kaydýr
    if(DS_SDA_BIT)  rdata |= 0x80;              //if(data hat degeri 1 ise)en degerli biti 1 yap
  } 
...
void BurstRead1302(unsigned char *ptr)
{
  ...
  for (i = 8; i > 0; i--) 
  {
    *ptr++ = Read1Byte(); //(bit banding)8 bit veri alýnýp pointer'a yazýyor pointer=>rdata
  }
  DS_SCL1;
  DS_RST0;
}
